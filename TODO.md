...
- [ ] **首頁 FAQ 背景可考慮加平板／手機直幅裁切** — 目前三個斷點共用
      `qa-bg.jpg`（1536x1024 橫幅）。設計稿建議 tablet 1024x1365、
      mobile 768x1365。要加的話把檔案命名為 `qa-bg_t.jpg`、`qa-bg_m.jpg`
      放進同一資料夾，再把 `_modules/m-faq.css` 裡註解掉的兩個
      `@screen` 區塊解開即可。
- [ ] **`_price-compare.ejs`**（學習渠道比價）目前停用但檔案保留在
...
2026-09-23：全站圖片由 115 MB 壓到 14 MB。照片一律改存 JPEG，
build 設定裡的 pngquant 也調過了 —— 它會把 PNG 降成 256 色調色盤，
那是給圖形用的，拿來壓照片會壓出色帶與雜點。

剩下比較重的是 contact 頁（25 張、4.7 MB）與 service 頁（12 張、3.6 MB）。
service 的十一張是直式手機截圖（1170x2532），沒有縮尺寸只換了格式；
若之後有橫幅的正式照片，換掉可以再省一半。

- [ ] contact 的 `pain-points-bg.jpg` 現在是 1280x1920、765 KB。
      原檔 4160x6240 在壓縮時被就地覆寫掉了，若還留著原圖，
      重新以 1920 寬匯出會更清楚（那塊是滿版橫幅，會被放大 1.5 倍）。
